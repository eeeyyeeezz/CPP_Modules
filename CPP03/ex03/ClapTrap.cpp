...
void ClapTrap::attack(std::string const &target){
	if (this->energyPoints >= 5){
		std::cout << BRIGHT_GREEN << this->name << " attacks " << target << ", causing " << this->attackDamage << " attack damage! (ง ͠° ͟ل͜ ͡°)ง" << NORMAL << std::endl;
		this->energyPoints -= 5;
	}
	else	
...
void ClapTrap::takeDamage(unsigned int ammount){
	...
	std::cout << YELLOW << this->name << " with " << temp << " hitpoints, takes " << ammount << " damage! Total hitPoints: " << this->hitPoints << NORMAL;
	if (this->hitPoints <= 0){
